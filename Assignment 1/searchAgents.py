...
class CapsuleSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        #setting start state
        self.startState = (startingGameState.getPacmanPosition(), self.foods, False)
        #setting capsule
    ...
    def getSuccessors(self, state):
        ...
        #basis of the code from getSuccessor in FoodSearchProblem
        #process the successors in the directions
        for direction in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            x, y = state[0]
            dx, dy = Actions.directionToVector(direction)
            nextx, nexty = int(x + dx), int(y + dy)
            # iniciate boolean from startState
            # start with capsule not eaten, false
            capsule_eaten = state[2]
            walls = self.walls[nextx][nexty]
            foods = self.foods[nextx][nexty]

            #check if the capsule is eaten, if yes, go to space that is not wall.
            if capsule_eaten:
                if not walls:
                    nextFood = state[1].copy()
                    nextFood[nextx][nexty] = False
                    successors.append((((nextx, nexty), nextFood, capsule_eaten), direction, 1))
            #else go to space that is not wall or not food.
            else:
                if not walls and not foods:
                    nextSpace = state[1].copy()
                    if (nextx, nexty) == self.capsule:
                        capsule_eaten = True
                    successors.append((((nextx, nexty), nextSpace, capsule_eaten), direction, 1))
        return successors

    #use to get capsule position (created)
    def getCapsule(self):
        return self.capsule
...
def capsuleProblemHeuristic(state, problem):
    ...
    location, food_grid, capsule_eaten = state
    h_value = 0
    if capsule_eaten:
        food_positions = food_grid.asList()
        for food_position in food_positions:
            h_value += util.manhattanDistance(location, food_position)
            ## other possible tests below
            # h_value = min(h_value, util.manhattanDistance(location, food_position))
            # h_value = max(h_value, util.manhattanDistance(location, food_position))
    else:
        h_value = util.manhattanDistance(location, problem.getCapsule())
    return h_value
